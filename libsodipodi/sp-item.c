...
static void sp_item_build (SPObject *object, SPDocument *document, TheraNode *node);
...
static void sp_item_update (SPObject *object, SPCtx *ctx, unsigned int flags);
static TheraNode *sp_item_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, unsigned int flags);
...
static void
sp_item_build (SPObject *object, SPDocument *document, TheraNode *node)
{
...
static void
sp_item_remove_clip (SPItem *item)
{
	if (item->clip) {
		SPItemView *v;
...
static void
sp_item_remove_mask (SPItem *item)
{
	if (item->mask) {
		SPItemView *v;
		for (v = item->display; v != NULL; v = v->view.next) {
...
sp_item_release (SPObject * object)
{
	...
	sp_item_remove_clip (item);
	sp_item_remove_mask (item);

	/* Children have to be released before hiding item */
	...
	case SP_PROP_CLIP_PATH: {
		...
		if (cp != item->clip) {
			...
			if (SP_IS_CLIPPATH (cp)) {
				...
				for (v = item->display; v != NULL; v = v->view.next) {
					NRArenaItem *ai;
					if (!NR_ARENA_ITEM_GET_KEY (item->display)) NR_ARENA_ITEM_SET_KEY (v, sp_item_display_key_new (3));
					ai = sp_clippath_show (SP_CLIPPATH (item->clip), NR_ARENA_ITEM_ARENA (v), NR_ARENA_ITEM_GET_KEY (v));
		...
		if (m != item->mask) {
			...
			if (SP_IS_MASK (m)) {
				...
				for (v = item->display; v != NULL; v = v->view.next) {
					NRArenaItem *ai;
					if (!NR_ARENA_ITEM_GET_KEY (item->display)) NR_ARENA_ITEM_SET_KEY (v, sp_item_display_key_new (3));
					ai = sp_mask_show (SP_MASK (item->mask), NR_ARENA_ITEM_ARENA (v), NR_ARENA_ITEM_GET_KEY (v));
...
static TheraNode *
sp_item_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, unsigned int flags)
{
	SPItem *item;
	guchar c[256];
	guchar *s;
	...
	if (sp_svg_transform_write (c, 256, &item->transform)) {
		thera_node_set_attribute (node, "transform", c);
	} else {
		thera_node_set_attribute (node, "transform", NULL);
	}

	if (SP_OBJECT_PARENT (object)) {
		s = sp_style_write_difference (SP_OBJECT_STYLE (object), SP_OBJECT_STYLE (SP_OBJECT_PARENT (object)));
		thera_node_set_attribute (node, "style", (s && *s) ? s : NULL);
		g_free (s);
	} else {
		thera_node_set_attribute (node, "style", NULL);
	}

	if (flags & SP_OBJECT_WRITE_SODIPODI) {
		thera_node_set_attribute (node, "sodipodi:insensitive", item->sensitive ? NULL : "true");
	}
...
void
sp_item_invoke_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int clear)
{
...
void
sp_item_invoke_bbox_full (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags, unsigned int clear)
{
	g_return_if_fail (item != NULL);
	g_return_if_fail (SP_IS_ITEM(item));
	g_return_if_fail (bbox != NULL);

	if (clear) nr_rect_f_set_empty (bbox);
...
sp_item_private_snappoints (SPItem *item, NRPointF *p, int size, const NRMatrixF *transform)
{
	NRRectF bbox;
	if (size < 4) return 0;
	sp_item_invoke_bbox (item, &bbox, transform, TRUE);
	p[0].x = bbox.x0;
	p[0].y = bbox.y0;
	p[1].x = bbox.x1;
	p[1].y = bbox.y0;
...
sp_item_invoke_show (SPItem *item, NRArena *arena, unsigned int key, unsigned int flags)
{
	...
	if (ai != NULL) {
		...
		if (item->clip) {
			NRArenaItem *ac;
			if (!NR_ARENA_ITEM_GET_KEY (item->display)) NR_ARENA_ITEM_SET_KEY (item->display, sp_item_display_key_new (3));
...
void
sp_item_write_transform (SPItem *item, TheraNode *node, NRMatrixF *transform)
{
	unsigned char c[80];
	NRMatrixF ltrans;
	...
	if (transform) {
		...
		if (((SPItemClass *) G_OBJECT_GET_CLASS (item))->write_transform)
			((SPItemClass *) G_OBJECT_GET_CLASS (item))->write_transform (item, node, &ltrans);
		transform = &ltrans;
	}

	if (sp_svg_transform_write (c, 80, transform)) {
		thera_node_set_attribute (((SPObject *) item)->node, "transform", c);
	} else {
		thera_node_set_attribute (((SPObject *) item)->node, "transform", NULL);
	}
...
sp_item_get_bbox_document (SPItem *item, NRRectF *bb, unsigned int flags, unsigned int clear)
{
	...
	sp_item_invoke_bbox_full (item, bb, &i2docf, flags, clear);
}
