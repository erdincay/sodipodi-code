...
static void sp_star_build (SPObject * object, SPDocument * document, TheraNode * node);
static TheraNode *sp_star_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags);
...
static void
sp_star_build (SPObject * object, SPDocument * document, TheraNode * node)
{
...
static TheraNode *
sp_star_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags)
{
	SPStar *star;
	...
	if ((flags & SP_OBJECT_WRITE_BUILD) && !node) {
		node = thera_document_new_element (thedoc, "polygon");
	}

	if (flags & SP_OBJECT_WRITE_SODIPODI) {
		thera_node_set_attribute (node, "sodipodi:type", "star");
		thera_node_set_int (node, "sodipodi:sides", star->sides);
		thera_node_set_double (node, "sodipodi:cx", star->cx);
		thera_node_set_double (node, "sodipodi:cy", star->cy);
		thera_node_set_double (node, "sodipodi:r1", star->r1);
		thera_node_set_double (node, "sodipodi:r2", star->r2);
		thera_node_set_double (node, "sodipodi:arg1", star->arg1);
		thera_node_set_double (node, "sodipodi:arg2", star->arg2);
	}
...
sp_star_set (SPObject *object, unsigned int key, const unsigned char *value)
{
	...
	case SP_ATTR_SODIPODI_CX:
		if (!sp_svg_length_read_uff (value, &unit, NULL, &star->cx) ||
		    (unit == SP_SVG_UNIT_EM) ||
		    (unit == SP_SVG_UNIT_EX) ||
	...
	case SP_ATTR_SODIPODI_CY:
		if (!sp_svg_length_read_uff (value, &unit, NULL, &star->cy) ||
		    (unit == SP_SVG_UNIT_EM) ||
		    (unit == SP_SVG_UNIT_EX) ||
		    (unit == SP_SVG_UNIT_PERCENT)) {
			star->cy = 0.0;
	...
	case SP_ATTR_SODIPODI_R1:
		if (!sp_svg_length_read_uff (value, &unit, NULL, &star->r1) ||
		    (unit == SP_SVG_UNIT_EM) ||
		    (unit == SP_SVG_UNIT_EX) ||
		    (unit == SP_SVG_UNIT_PERCENT)) {
			star->r1 = 1.0;
	...
	case SP_ATTR_SODIPODI_R2:
		if (!sp_svg_length_read_uff (value, &unit, NULL, &star->r2) ||
		    (unit == SP_SVG_UNIT_EM) ||
		    (unit == SP_SVG_UNIT_EX) ||
		    (unit == SP_SVG_UNIT_PERCENT)) {
			star->r2 = 0.0;
...
sp_star_set_shape (SPShape *shape)
{
	...
	nr_dynamic_path_moveto (&dpath, p.x, p.y);
	sp_star_get_xy (star, SP_STAR_POINT_KNOT2, 0, &p);
	nr_dynamic_path_lineto (&dpath, p.x, p.y);
	...
	sp_shape_set_curve_insync (SP_SHAPE (star), dpath.path, TRUE);
}
...
sp_star_get_xy (SPStar *star, SPStarPoint point, gint index, NRPointF *p)
{
	...
	darg = NR_2PI_F / (double) star->sides;
