...
#include <libnrarena/nr-arena-glyphs.h>
...
static void sp_chars_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags);
...
sp_chars_modified (SPObject *object, unsigned int flags)
{
	...
	if (flags & SP_OBJECT_STYLE_MODIFIED_FLAG) {
		...
		for (v = SP_ITEM (chars)->display; v != NULL; v = v->view.next) {
			...
			nr_arena_glyphs_group_set_style (NR_ARENA_GLYPHS_GROUP (v), arenastyle);
		}
	}
}

static void
sp_chars_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags)
{
	SPChars *chars;
	SPCharElement *el;
	...
	for (el = chars->elements; el != NULL; el = el->next) {
		...
		if (path) {
			NRMatrixF a;
			nr_matrix_multiply_fff (&a, &el->transform, transform);
...
sp_chars_show (SPItem *item, NRArena *arena, unsigned int key, unsigned int flags)
{
	...
	arenastyle = sp_style_get_arena_style (SP_OBJECT_STYLE (item));
	nr_arena_glyphs_group_set_style (NR_ARENA_GLYPHS_GROUP (arenaitem), arenastyle);
...
sp_chars_normalized_bpath_list (SPChars *chars)
{
	...
	for (el = chars->elements; el != NULL; el = el->next) {
		...
		if (bp) {
			...
			if (tpath) cc = g_slist_prepend (cc, tpath);
		}
	}
