...
static void sp_clippath_build (SPObject *object, SPDocument *document, TheraNode *node);
...
static SPObject *sp_clippath_child_added (SPObject *object, TheraNode *child, TheraNode *ref);
static unsigned int sp_clippath_remove_child (SPObject *object, TheraNode *child);
...
static TheraNode *sp_clippath_write (SPObject *object, TheraDocument *thedoc, TheraNode *repr, guint flags);
...
static void
sp_clippath_build (SPObject *object, SPDocument *document, TheraNode *node)
{
	SPClipPath *cp;
...
static SPObject *
sp_clippath_child_added (SPObject *object, TheraNode *thechild, TheraNode *ref)
{
	SPClipPath *cp;
	SPObject *child;
	...
	if (SP_IS_ITEM (child)) {
		...
		for (v = cp->display; v != NULL; v = v->next) {
			NRArenaItem *ac;
			ac = sp_item_invoke_show (SP_ITEM (child), NR_ARENA_ITEM_ARENA (v->arenaitem), v->key, SP_ITEM_REFERENCE_FLAGS);
			if (ac) {
...
static unsigned int
sp_clippath_remove_child (SPObject *object, TheraNode *child)
{
	SPClipPath *cp;
...
static TheraNode *
sp_clippath_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags)
{
	SPClipPath *cp;
	...
	if (!node && (flags & SP_OBJECT_WRITE_BUILD)) {
		node = thera_document_new_node (thedoc, THERA_NODE_ELEMENT, "clipPath");
	}
