...
static NRDash *sp_style_read_dash (NRDash *dash, const guchar *str);
...
#endif
static void sp_style_read_penum (SPIEnum *val, TheraNode *node, const char *key, const SPStyleEnum *dict, unsigned int inherit);
static void sp_style_read_plength (SPILength *val, TheraNode *node, const char *key);
static void sp_style_read_pfontsize (SPIFontSize *val, TheraNode *node, const char *key);
...
static void sp_style_update_arena_style (SPStyle *style);
...
/* 		if (style->object) gtk_signal_disconnect_by_data (GTK_OBJECT (style->object), style); */
		...
		if (style->stroke_dash) free (style->stroke_dash);
		if (style->arenastyle) nr_arena_style_unref (style->arenastyle);
		g_free (style);
	}
...
static void
sp_style_read (SPStyle *style, SPObject *object, TheraNode *node)
{
	const guchar *val;

	g_assert (style != NULL);
	g_assert (node != NULL);
	g_assert (!object || (object->node == node));

	sp_style_clear (style);

	/* 1. Style itself */
	val = thera_node_get_attribute (node, "style");
	if (val != NULL) {
	...
	if (!style->fill.set || (style->fill.type == SP_PAINT_TYPE_COLOR)) {
		...
		val = thera_node_get_attribute (node, "fill-cmyk");
		if (val) {
			thera_node_set_attribute (node, "sodipodi:fill-cmyk", val);
			thera_node_set_attribute (node, "fill-cmyk", NULL);
		}
		val = thera_node_get_attribute (node, "sodipodi:fill-cmyk");
	...
	if (!style->stroke.set || (style->stroke.type == SP_PAINT_TYPE_COLOR)) {
		/* backwards-compatability */
		val = thera_node_get_attribute (node, "stroke-cmyk");
		if (val) {
			thera_node_set_attribute (node, "sodipodi:stroke-cmyk", val);
			thera_node_set_attribute (node, "stroke-cmyk", NULL);
		}
		val = thera_node_get_attribute (node, "sodipodi:stroke-cmyk");
	...
	/* CSS2 */
	/* Font */
	SPS_READ_PFONTSIZE_IF_UNSET (&style->font_size, node, "font-size");
	SPS_READ_PENUM_IF_UNSET (&style->font_style, node, "font-style", enum_font_style, TRUE);
	SPS_READ_PENUM_IF_UNSET (&style->font_variant, node, "font-variant", enum_font_variant, TRUE);
	SPS_READ_PENUM_IF_UNSET (&style->font_weight, node, "font-weight", enum_font_weight, TRUE);
	SPS_READ_PENUM_IF_UNSET (&style->font_stretch, node, "font-stretch", enum_font_stretch, TRUE);
	/* opacity */
	if (!style->opacity.set) {
		val = thera_node_get_attribute (node, "opacity");
		if (val) {
	...
	/* color */
	if (!style->color.set) {
		val = thera_node_get_attribute (node, "color");
		if (val) {
	...
	/* fill */
	if (!style->fill.set) {
		val = thera_node_get_attribute (node, "fill");
		if (val) {
	...
	if (!style->fill_opacity.set) {
		val = thera_node_get_attribute (node, "fill-opacity");
		if (val) {
			sp_style_read_iscale24 (&style->fill_opacity, val);
		}
	}
	/* fill-rule */
	SPS_READ_PENUM_IF_UNSET (&style->fill_rule, node, "fill-rule", enum_fill_rule, TRUE);
	/* stroke */
	if (!style->stroke.set) {
		val = thera_node_get_attribute (node, "stroke");
		if (val) {
			sp_style_read_ipaint (&style->stroke, val, style, (object) ? SP_OBJECT_DOCUMENT (object) : NULL);
		}
	}
	SPS_READ_PLENGTH_IF_UNSET (&style->stroke_width, node, "stroke-width");
	SPS_READ_PENUM_IF_UNSET (&style->stroke_linecap, node, "stroke-linecap", enum_stroke_linecap, TRUE);
	SPS_READ_PENUM_IF_UNSET (&style->stroke_linejoin, node, "stroke-linejoin", enum_stroke_linejoin, TRUE);
	/* stroke-opacity */
	if (!style->stroke_opacity.set) {
		val = thera_node_get_attribute (node, "stroke-opacity");
		if (val) {
	...
	if (!style->stroke_dasharray_set) {
		...
		style->stroke_dash = sp_style_read_dash (style->stroke_dash, val);
	...
	if (!style->text_private || !style->text->font_family.set) {
		val = thera_node_get_attribute (node, "font-family");
		if (val) {
	...
	/* SVG */
	SPS_READ_PENUM_IF_UNSET (&style->text_anchor, node, "text-anchor", enum_text_anchor, TRUE);
	SPS_READ_PENUM_IF_UNSET (&style->writing_mode, node, "writing-mode", enum_writing_mode, TRUE);
	...
	if (object) {
		...
		if (thera_node_get_parent (node)) {
			SPStyle *parent;
			/* fixme: This is not the prettiest thing (Lauris) */
			parent = sp_style_new ();
			sp_style_read (parent, NULL, thera_node_get_parent (node));
...
sp_style_read_from_object (SPStyle *style, SPObject *object)
{
	...
	sp_style_read (style, object, object->node);
}

void
sp_style_read_from_node (SPStyle *style, TheraNode *node)
{
	g_return_if_fail (style != NULL);
	g_return_if_fail (node != NULL);

	sp_style_read (style, NULL, node);
}
...
sp_style_merge_from_parent (SPStyle *style, SPStyle *parent)
{
	...
	if (!style->font_size.set || style->font_size.inherit) {
		...
		if (style->font_size.value < SP_CSS_FONT_SIZE_SMALLER) {
			...
			style->font_size.computed = parent->font_size.computed / 1.2f;
	...
	if (!style->stroke_width.set || style->stroke_width.inherit) {
		...
		style->stroke_width.computed = style->font_size.computed * 0.5f;
	}
...
sp_style_clear (SPStyle *style)
{
	...
	sp_color_set_rgb_float (&style->color.value.color, 0.0, 0.0, 0.0, TRUE, FALSE);

	style->fill.type = SP_PAINT_TYPE_COLOR;
	sp_color_set_rgb_float (&style->fill.value.color, 0.0, 0.0, 0.0, TRUE, FALSE);
	...
	sp_color_set_rgb_float (&style->stroke.value.color, 0.0, 0.0, 0.0, TRUE, FALSE);
...
static NRDash *
sp_style_read_dash (NRDash *dash, const guchar *str)
{
#if 0
	gint n_dash;
	gdouble d[64];
...
sp_style_set_fill_color_rgba (SPStyle *style, gfloat r, gfloat g, gfloat b, gfloat a, unsigned int fill_set, unsigned int opacity_set)
{
	...
	style->fill.inherit = FALSE;
	style->fill.type = SP_PAINT_TYPE_COLOR;
	sp_color_set_rgb_float (&style->fill.value.color, r, g, b, TRUE, FALSE);
...
sp_style_set_fill_color_cmyka (SPStyle *style, gfloat c, gfloat m, gfloat y, gfloat k, gfloat a, unsigned int fill_set, unsigned int opacity_set)
{
	...
	style->fill.inherit = FALSE;
	style->fill.type = SP_PAINT_TYPE_COLOR;
	sp_color_set_cmyk_float (&style->fill.value.color, c, m, y, k, TRUE, FALSE);
	style->fill_opacity.set = opacity_set;
	style->fill_opacity.inherit = FALSE;
	style->fill_opacity.value = SP_SCALE24_FROM_FLOAT (a);
...
sp_style_set_stroke_color_rgba (SPStyle *style, gfloat r, gfloat g, gfloat b, gfloat a, unsigned int stroke_set, unsigned int opacity_set)
{
	...
	sp_color_set_rgb_float (&style->stroke.value.color, r, g, b, TRUE, FALSE);
...
sp_style_set_stroke_color_cmyka (SPStyle *style, gfloat c, gfloat m, gfloat y, gfloat k, gfloat a, unsigned int stroke_set, unsigned int opacity_set)
{
	...
	style->stroke.inherit = FALSE;
	style->stroke.type = SP_PAINT_TYPE_COLOR;
	sp_color_set_cmyk_float (&style->stroke.value.color, c, m, y, k, TRUE, FALSE);
	style->stroke_opacity.set = opacity_set;
	style->stroke_opacity.inherit = FALSE;
	style->stroke_opacity.value = SP_SCALE24_FROM_FLOAT (a);
	...
	if (!strcmp (str, "inherit")) {
		...
		if ((const guchar *) e != str) {
			if (!*e) {
				...
				val->computed = (float) value * 1.25f;
			} else if (!strcmp (e, "pc")) {
				/* Userspace * 15 */
				val->unit = SP_CSS_UNIT_PC;
				val->computed = (float) value * 15.0f;
			} else if (!strcmp (e, "mm")) {
				/* Userspace * 3.543307 */
				val->unit = SP_CSS_UNIT_MM;
				val->computed = (float) value * 3.543307f;
			} else if (!strcmp (e, "cm")) {
				/* Userspace * 35.43307 */
				val->unit = SP_CSS_UNIT_CM;
				val->computed = (float) value * 35.43307f;
			} else if (!strcmp (e, "in")) {
				/* Userspace * 90 */
				val->unit = SP_CSS_UNIT_IN;
				val->computed = (float) value * 90.0f;
			} else if (!strcmp (e, "em")) {
				/* EM square */
...
static void
sp_style_read_penum (SPIEnum *val, TheraNode *node, const char *key, const SPStyleEnum *dict, unsigned int inherit)
{
	const guchar *str;
	str = thera_node_get_attribute (node, key);
	if (str) {
...
static void
sp_style_read_plength (SPILength *val, TheraNode *node, const char *key)
{
	const guchar *str;
	str = thera_node_get_attribute (node, key);
	if (str) {
...
static void
sp_style_read_pfontsize (SPIFontSize *val, TheraNode *node, const char *key)
{
	const guchar *str;
	str = thera_node_get_attribute (node, key);
	if (str) {
	...
	if (((flags & SP_STYLE_FLAG_IFSET) && val->set) ||
		...
		if (val->inherit) {
			...
			case SP_CSS_UNIT_PX:
				...
				break;
			case SP_CSS_UNIT_PT:
				r = val->computed / 1.25f;
				u = "pt";
				break;
			case SP_CSS_UNIT_PC:
				r = val->computed / 15.0f;
				u = "pc";
				break;
			case SP_CSS_UNIT_MM:
				r = val->computed / 3.543307f;
				u = "mm";
				break;
			case SP_CSS_UNIT_CM:
				r = val->computed / 35.43307f;
				u = "cm";
				break;
			case SP_CSS_UNIT_IN:
				r = val->computed / 90.0f;
				u = "in";
				break;
			case SP_CSS_UNIT_EM:
				r = val->value;
	...
	if (eptr && (eptr != cptr)) {
		...
		if (eptr && (eptr != cptr)) {
			...
			if (eptr && (eptr != cptr)) {
				...
				if (eptr && (eptr != cptr)) {
					sp_color_set_cmyk_float (color, (float) c, (float) m, (float) y, (float) k, TRUE, FALSE);
					return color;
				}
			}
		}
...
NRArenaStyle *
sp_style_get_arena_style (SPStyle *style)
{
	if (!style->arenastyle) {
		style->arenastyle = nr_arena_style_new ();
		sp_style_update_arena_style (style);
	}
	return style->arenastyle;
}

static void
sp_style_update_arena_style (SPStyle *style)
{
	style->arenastyle->opacity = SP_SCALE24_TO_FLOAT (style->opacity.value);
	if (style->fill.type == SP_PAINT_TYPE_COLOR) {
		style->arenastyle->fill.type = NR_ARENA_PAINT_COLOR;
		style->arenastyle->fill.color.rgba = sp_color_get_rgba32_falpha (&style->fill.value.color, SP_SCALE24_TO_FLOAT (style->fill_opacity.value));
	} else if (style->fill.type == SP_PAINT_TYPE_PAINTSERVER) {
		style->arenastyle->fill.type = NR_ARENA_PAINT_SERVER;
		/* fixme: Here we have to create uninitialized renderer and later set it up */
		style->arenastyle->fill.server = NR_PAINT_SERVER(sp_paint_server_painter_new (style->fill.value.server));
	} else {
		style->arenastyle->fill.type = NR_ARENA_PAINT_NONE;
	}
	if (style->stroke.type == SP_PAINT_TYPE_COLOR) {
		style->arenastyle->stroke.type = NR_ARENA_PAINT_COLOR;
		style->arenastyle->stroke.color.rgba = sp_color_get_rgba32_falpha (&style->stroke.value.color, SP_SCALE24_TO_FLOAT (style->stroke_opacity.value));
	} else {
		style->arenastyle->stroke.type = NR_ARENA_PAINT_NONE;
	}
	if (style->fill_rule.value == SP_WIND_RULE_NONZERO) {
		style->arenastyle->fill_rule = NR_WIND_RULE_NONZERO;
	} else {
		style->arenastyle->fill_rule = NR_WIND_RULE_EVENODD;
	}
	style->arenastyle->stroke_width = style->stroke_width.computed;
	if (style->stroke_linecap.computed == SP_STROKE_LINECAP_BUTT) {
		style->arenastyle->stroke_linecap = NR_STROKE_CAP_BUTT;
	} else if (style->stroke_linecap.computed == SP_STROKE_LINECAP_ROUND) {
		style->arenastyle->stroke_linecap = NR_STROKE_CAP_ROUND;
	} else if (style->stroke_linecap.computed == SP_STROKE_LINECAP_SQUARE) {
		style->arenastyle->stroke_linecap = NR_STROKE_CAP_SQUARE;
	}
	if (style->stroke_linejoin.computed == SP_STROKE_LINEJOIN_MITER) {
		style->arenastyle->stroke_linejoin = NR_STROKE_JOIN_MITER;
	} else if (style->stroke_linejoin.computed == SP_STROKE_LINEJOIN_ROUND) {
		style->arenastyle->stroke_linejoin = NR_STROKE_JOIN_ROUND;
	} else if (style->stroke_linejoin.computed == SP_STROKE_LINEJOIN_BEVEL) {
		style->arenastyle->stroke_linejoin = NR_STROKE_JOIN_BEVEL;
	}
	style->arenastyle->stroke_miterlimit = style->stroke_miterlimit.value;
}
