...
#define NR_ARENA_ITEM_VIRTUAL(i,m) (((NRArenaItemClass *) ARIKKEI_OBJECT_GET_CLASS (i))->m)
...
struct _NRArenaItem {
	...
	/* Item key designating the owner of secondary display objects (clippaths, masks) */
	unsigned int key;
#ifdef NR_ARENA_ITEM_HAS_VIEW
...
struct _NRArenaItemClass {
	...
	unsigned int (* update) (NRArenaItem *item, const NRRectL *area, const NRGC *gc, unsigned int state, unsigned int reset);
	unsigned int (* render) (NRArenaItem *item, const NRRectL *area, NRPixBlock *pb, unsigned int flags);
	unsigned int (* clip) (NRArenaItem *item, const NRRectL *area, NRPixBlock *pb);
	NRArenaItem * (* pick) (NRArenaItem *item, float x, float y, float delta, unsigned int sticky);
};
...
unsigned int nr_arena_item_invoke_update (NRArenaItem *item, const NRRectL *area, const NRGC *gc, unsigned int state, unsigned int reset);
...
unsigned int nr_arena_item_invoke_render (NRArenaItem *item, const NRRectL *area, NRPixBlock *pb, unsigned int flags);

unsigned int nr_arena_item_invoke_clip (NRArenaItem *item, const NRRectL *area, NRPixBlock *pb);
NRArenaItem *nr_arena_item_invoke_pick (NRArenaItem *item, float x, float y, float delta, unsigned int sticky);
...
#ifdef NR_ARENA_ITEM_HAS_VIEW
#define NR_ARENA_ITEM_VIEW_NEXT(i) ((i)->view.next)
