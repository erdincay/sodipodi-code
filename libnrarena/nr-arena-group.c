...
static void nr_arena_group_finalize (ArikkeiObject *object);
...
static unsigned int nr_arena_group_update (NRArenaItem *item, const NRRectL *area, const NRGC *gc, unsigned int state, unsigned int reset);
static unsigned int nr_arena_group_render (NRArenaItem *item, const NRRectL *area, NRPixBlock *pb, unsigned int flags);
static unsigned int nr_arena_group_clip (NRArenaItem *item, const NRRectL *area, NRPixBlock *pb);
static NRArenaItem *nr_arena_group_pick (NRArenaItem *item, float x, float y, float delta, unsigned int sticky);
...
nr_arena_group_get_type (void)
{
	...
	if (!type) {
		type = arikkei_object_register_type (NR_TYPE_ARENA_ITEM,
						...
						sizeof (NRArenaGroup),
						(void (*) (ArikkeiObjectClass *)) nr_arena_group_class_init,
						(void (*) (ArikkeiObject *)) nr_arena_group_init);
	}
	return type;
}
...
static void
nr_arena_group_finalize (ArikkeiObject *object)
{
	NRArenaItem *item;
	NRArenaGroup *group;
...
static unsigned int
nr_arena_group_update (NRArenaItem *item, const NRRectL *area, const NRGC *gc, unsigned int state, unsigned int reset)
{
	NRArenaGroup *group;
	NRArenaItem *child;
	...
	for (child = group->children; child != NULL; child = child->next) {
		NRGC cgc;
		nr_matrix_multiply_fff (&cgc.transform, &group->child_transform, &gc->transform);
...
static unsigned int
nr_arena_group_render (NRArenaItem *item, const NRRectL *area, NRPixBlock *pb, unsigned int flags)
{
	NRArenaGroup *group;
	NRArenaItem *child;
	unsigned int ret;
...
static unsigned int
nr_arena_group_clip (NRArenaItem *item, const NRRectL *area, NRPixBlock *pb)
{
	NRArenaGroup *group;
	NRArenaItem *child;
	unsigned int ret;
...
static NRArenaItem *
nr_arena_group_pick (NRArenaItem *item, float x, float y, float delta, unsigned int sticky)
{
	NRArenaGroup *group;
...
{
	arikkei_return_if_fail (group != NULL);
	arikkei_return_if_fail (NR_IS_ARENA_GROUP (group));

	group->transparent = transparent;
}

void
nr_arena_group_set_child_transform (NRArenaGroup *group, const NRMatrixF *t)
{
