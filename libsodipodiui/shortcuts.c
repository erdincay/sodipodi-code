...
sp_shortcut_table_load (const unsigned char *name)
{
	...
	sp_shortcut_set_verb (SP_SHORTCUT_CONTROL_MASK | GDK_q, SP_VERB_FILE_EXIT, TRUE);
	sp_shortcut_set_verb (SP_SHORTCUT_CONTROL_MASK | GDK_Q, SP_VERB_FILE_EXIT, FALSE);
	/* Event contexts */
...
sp_shortcut_set_verb (unsigned int shortcut, unsigned int verb, unsigned int primary)
{
	...
	ex = GPOINTER_TO_UINT(g_hash_table_lookup (scdict, GUINT_TO_POINTER(shortcut)));
	if (ex != verb) g_hash_table_insert (scdict, GUINT_TO_POINTER(shortcut), GUINT_TO_POINTER(verb));
	if (primary) {
		SPAction *action;
		action = sp_action_lookup_by_verb (verb);
...
sp_shortcut_remove_verb (unsigned int shortcut)
{
	...
	if (ex) {
		SPAction *action;
		g_hash_table_insert (scdict, GUINT_TO_POINTER(shortcut), NULL);
		action = sp_action_lookup_by_verb (ex);
...
{
	if (!scdict) return 0;
	return GPOINTER_TO_UINT(g_hash_table_lookup (scdict, GUINT_TO_POINTER(shortcut)));
}
