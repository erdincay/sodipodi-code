...
{
	struct _NRNodePathBuildData ndata;
	unsigned int nsegments, size, i;
	nsegments = nr_path_get_num_segments (path);
	size = sizeof (struct _NRNodePath) + nsegments * sizeof (struct _NRNodeSeg) - sizeof (struct _NRNodeSeg);
	ndata.npath = (struct _NRNodePath *) malloc (size);
	ndata.npath->nsegs = nsegments;
