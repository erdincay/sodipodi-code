...
sp_color_selector_init (SPColorSelector *csel)
{
	...
	for (i = 0; i < 5; i++) {
		...
		/* Adjustment */
		csel->a[i] = (GtkAdjustment *) gtk_adjustment_new (0.0, 0.0, 1.0, 0.01, 0.1, 0);
		/* Slider */
...
sp_color_selector_get_color_alpha (SPColorSelector *cs, SPColor *color, gfloat *alpha)
{
	...
	case SP_COLOR_SELECTOR_MODE_HSV:
		...
		sp_color_set_rgb_float (color, c[0], c[1], c[2], TRUE, FALSE);
		if (alpha) *alpha = c[3];
		break;
	case SP_COLOR_SELECTOR_MODE_CMYK:
		sp_color_selector_get_cmyka_floatv (cs, c);
		sp_color_set_cmyk_float (color, c[0], c[1], c[2], c[3], TRUE, FALSE);
		if (alpha) *alpha = c[4];
		break;
	default:
		g_warning ("file %s: line %d: Illegal color selector mode %d", __FILE__, __LINE__, cs->mode);
...
sp_color_selector_get_rgba_floatv (SPColorSelector *csel, gfloat *rgba)
{
	...
	switch (csel->mode) {
	case SP_COLOR_SELECTOR_MODE_RGB:
		rgba[0] = (float) csel->a[0]->value;
		rgba[1] = (float) csel->a[1]->value;
		rgba[2] = (float) csel->a[2]->value;
		rgba[3] = (float) csel->a[3]->value;
		break;
	case SP_COLOR_SELECTOR_MODE_HSV:
		sp_color_hsv_to_rgb_floatv (rgba, (float) csel->a[0]->value, (float) csel->a[1]->value, (float) csel->a[2]->value);
		rgba[3] = (float) csel->a[3]->value;
		break;
	case SP_COLOR_SELECTOR_MODE_CMYK:
		sp_color_cmyk_to_rgb_floatv (rgba, (float) csel->a[0]->value, (float) csel->a[1]->value, (float) csel->a[2]->value, (float) csel->a[3]->value);
		rgba[3] = (float) csel->a[4]->value;
		break;
	default:
...
sp_color_selector_get_cmyka_floatv (SPColorSelector *csel, gfloat *cmyka)
{
	...
	switch (csel->mode) {
	case SP_COLOR_SELECTOR_MODE_RGB:
		sp_color_rgb_to_cmyk_floatv (cmyka, (float) csel->a[0]->value, (float) csel->a[1]->value, (float) csel->a[2]->value);
		cmyka[4] = (float) csel->a[3]->value;
		break;
	case SP_COLOR_SELECTOR_MODE_HSV:
		sp_color_hsv_to_rgb_floatv (rgb, (float) csel->a[0]->value, (float) csel->a[1]->value, (float) csel->a[2]->value);
		sp_color_rgb_to_cmyk_floatv (cmyka, rgb[0], rgb[1], rgb[2]);
		cmyka[4] = (float) csel->a[3]->value;
		break;
	case SP_COLOR_SELECTOR_MODE_CMYK:
		cmyka[0] = (float) csel->a[0]->value;
		cmyka[1] = (float) csel->a[1]->value;
		cmyka[2] = (float) csel->a[2]->value;
		cmyka[3] = (float) csel->a[3]->value;
		cmyka[4] = (float) csel->a[4]->value;
		break;
	default:
		g_warning ("file %s: line %d: Illegal color selector mode", __FILE__, __LINE__);
		break;
...
sp_color_selector_get_rgba32 (SPColorSelector *csel)
{
	...
	g_return_val_if_fail (SP_IS_COLOR_SELECTOR (csel), 0);
...
sp_color_selector_update_sliders (SPColorSelector *csel, guint channels)
{
	...
	case SP_COLOR_SELECTOR_MODE_HSV:
		...
		if ((channels != CSEL_CHANNEL_S) && (channels != CSEL_CHANNEL_A)) {
			/* Update saturation */
			sp_color_hsv_to_rgb_floatv (rgb0, (float) csel->a[0]->value, 0.0f, (float) csel->a[2]->value);
			sp_color_hsv_to_rgb_floatv (rgb1, (float) csel->a[0]->value, 1.0f, (float) csel->a[2]->value);
			sp_color_slider_set_colors (SP_COLOR_SLIDER (csel->s[1]),
		...
		if ((channels != CSEL_CHANNEL_V) && (channels != CSEL_CHANNEL_A)) {
			/* Update value */
			sp_color_hsv_to_rgb_floatv (rgb0, (float) csel->a[0]->value, (float) csel->a[1]->value, 0.0f);
			sp_color_hsv_to_rgb_floatv (rgb1, (float) csel->a[0]->value, (float) csel->a[1]->value, 1.0f);
			sp_color_slider_set_colors (SP_COLOR_SLIDER (csel->s[2]),
						    SP_RGBA32_F_COMPOSE (rgb0[0], rgb0[1], rgb0[2], 1.0),
						    SP_RGBA32_F_COMPOSE (rgb1[0], rgb1[1], rgb1[2], 1.0));
		}
		if (channels != CSEL_CHANNEL_A) {
			/* Update alpha */
			sp_color_hsv_to_rgb_floatv (rgb0, (float) csel->a[0]->value, (float) csel->a[1]->value, (float) csel->a[2]->value);
			sp_color_slider_set_colors (SP_COLOR_SLIDER (csel->s[3]),
	...
	case SP_COLOR_SELECTOR_MODE_CMYK:
		if ((channels != CSEL_CHANNEL_C) && (channels != CSEL_CHANNEL_CMYKA)) {
			/* Update saturation */
			sp_color_cmyk_to_rgb_floatv (rgb0, 0.0f, (float) csel->a[1]->value, (float) csel->a[2]->value, (float) csel->a[3]->value);
			sp_color_cmyk_to_rgb_floatv (rgb1, 1.0f, (float) csel->a[1]->value, (float) csel->a[2]->value, (float) csel->a[3]->value);
			sp_color_slider_set_colors (SP_COLOR_SLIDER (csel->s[0]),
						    SP_RGBA32_F_COMPOSE (rgb0[0], rgb0[1], rgb0[2], 1.0),
						    SP_RGBA32_F_COMPOSE (rgb1[0], rgb1[1], rgb1[2], 1.0));
		}
		if ((channels != CSEL_CHANNEL_M) && (channels != CSEL_CHANNEL_CMYKA)) {
			/* Update saturation */
			sp_color_cmyk_to_rgb_floatv (rgb0, (float) csel->a[0]->value, 0.0f, (float) csel->a[2]->value, (float) csel->a[3]->value);
			sp_color_cmyk_to_rgb_floatv (rgb1, (float) csel->a[0]->value, 1.0f, (float) csel->a[2]->value, (float) csel->a[3]->value);
			sp_color_slider_set_colors (SP_COLOR_SLIDER (csel->s[1]),
						    SP_RGBA32_F_COMPOSE (rgb0[0], rgb0[1], rgb0[2], 1.0),
						    SP_RGBA32_F_COMPOSE (rgb1[0], rgb1[1], rgb1[2], 1.0));
		}
		if ((channels != CSEL_CHANNEL_Y) && (channels != CSEL_CHANNEL_CMYKA)) {
			/* Update saturation */
			sp_color_cmyk_to_rgb_floatv (rgb0, (float) csel->a[0]->value, (float) csel->a[1]->value, 0.0f, (float) csel->a[3]->value);
			sp_color_cmyk_to_rgb_floatv (rgb1, (float) csel->a[0]->value, (float) csel->a[1]->value, 1.0f, (float) csel->a[3]->value);
			sp_color_slider_set_colors (SP_COLOR_SLIDER (csel->s[2]),
						    SP_RGBA32_F_COMPOSE (rgb0[0], rgb0[1], rgb0[2], 1.0),
						    SP_RGBA32_F_COMPOSE (rgb1[0], rgb1[1], rgb1[2], 1.0));
		}
		if ((channels != CSEL_CHANNEL_K) && (channels != CSEL_CHANNEL_CMYKA)) {
			/* Update saturation */
			sp_color_cmyk_to_rgb_floatv (rgb0, (float) csel->a[0]->value, (float) csel->a[1]->value, (float) csel->a[2]->value, 0.0f);
			sp_color_cmyk_to_rgb_floatv (rgb1, (float) csel->a[0]->value, (float) csel->a[1]->value, (float) csel->a[2]->value, 1.0f);
			sp_color_slider_set_colors (SP_COLOR_SLIDER (csel->s[3]),
						    SP_RGBA32_F_COMPOSE (rgb0[0], rgb0[1], rgb0[2], 1.0),
						    SP_RGBA32_F_COMPOSE (rgb1[0], rgb1[1], rgb1[2], 1.0));
		}
		if (channels != CSEL_CHANNEL_CMYKA) {
			/* Update saturation */
			sp_color_cmyk_to_rgb_floatv (rgb0, (float) csel->a[0]->value, (float) csel->a[1]->value, (float) csel->a[2]->value, (float) csel->a[3]->value);
