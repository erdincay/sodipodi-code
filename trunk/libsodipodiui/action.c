...
sp_action_finalize (NRObject *object)
{
	...
	if (actions) g_hash_table_remove (actions, (gconstpointer) &action->verb);
...
SPAction *
sp_action_setup (SPAction *action, unsigned int verb, const unsigned char *id, const unsigned char *name, const unsigned char *tip, const unsigned char *image)
{
	...
	action->verb = verb;
	if (!actions) actions = g_hash_table_new (g_int_hash, g_int_equal);
	g_hash_table_insert (actions, (gpointer) &action->verb, action);
...
SPAction *
sp_action_lookup_by_verb (unsigned int verb)
{
	if (!actions) return NULL;
	return (SPAction *) g_hash_table_lookup (actions, (gconstpointer) &verb);
}
