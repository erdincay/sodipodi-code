...
sp_button_button_release (GtkWidget *widget, GdkEventButton *event)
{
	...
	button = SP_BUTTON (widget);
	action = button->options[button->option].action;
	params = button->options[button->option].params;
	...
	if (event->button == 1) {
		...
		case SP_BUTTON_TYPE_NORMAL:
			...
			g_signal_emit (button, button_signals[RELEASED], 0);
			g_signal_emit (button, button_signals[CLICKED], 0);
			sp_action_perform (action, params);
			break;
		case SP_BUTTON_TYPE_TOGGLE:
			button->down = !button->initial;
			sp_button_update_state (button);
			...
			g_signal_emit (button, button_signals[TOGGLED], 0);
			sp_action_set_active (action, button->down);
			if (button->down) sp_action_perform (action, params);
			break;
		case SP_BUTTON_TYPE_SELECT:
			button->down = 1;
			sp_button_update_state (button);
...
GtkWidget *
sp_button_new (unsigned int size, unsigned int type, SPAction *action, void *params, GtkTooltips *tooltips)
{
	SPButton *button;
	...
	sp_button_add_option (button, 0, action, params);
...
void
sp_button_add_option (SPButton *button, unsigned int option, SPAction *action, void *params)
{
	...
	button->options[option].params = params;
			 ...
			 GtkTooltips *tooltips)
{
	...
	sp_action_setup (action, SP_VERB_NONE, name, name, tip, name);
