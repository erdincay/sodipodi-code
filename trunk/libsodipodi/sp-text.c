...
/* fixme: */
unsigned int
sp_repr_set_content_ucs2 (TheraNode *node, const NRUShort *newcontent)
{
	char *utf8;
	unsigned int retval;

	utf8 = arikkei_ucs2_utf8_strdup (newcontent);

	retval = thera_node_set_text_content (node, utf8);
...
static void sp_string_build (SPObject *object, SPDocument *document, TheraNode *node);
...
static void sp_string_set_shape (SPString *string, SPLayoutData *ly, NRPointF *cp, gboolean *inspace);
...
static void
sp_string_build (SPObject *object, SPDocument *doc, TheraNode *node)
{
	if (((SPObjectClass *) string_parent_class)->build)
		((SPObjectClass *) string_parent_class)->build (object, doc, node);
...
sp_string_read_content (SPObject *object)
{
	...
	t = thera_node_get_text_content (object->node);
	...
	if (t) {
		...
		string->uchars = (NRUShort *) malloc ((string->ulen + 1) * sizeof (NRUShort));
...
sp_string_calculate_dimensions (SPString *string)
{
	...
	string->bbox.x0 = string->bbox.y0 = NR_HUGE_F;
	string->bbox.x1 = string->bbox.y1 = -NR_HUGE_F;
...
static void
sp_string_set_shape (SPString *string, SPLayoutData *ly, NRPointF *cp, gboolean *pinspace)
{
	SPChars *chars;
	SPStyle *style;
	gint len;
...
static void sp_tspan_build (SPObject * object, SPDocument * document, TheraNode * node);
...
static SPObject *sp_tspan_child_added (SPObject *object, TheraNode *rch, TheraNode *ref);
static unsigned int sp_tspan_remove_child (SPObject *object, TheraNode *rch);
...
static TheraNode *sp_tspan_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags);

static void sp_tspan_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags);
...
static void sp_tspan_set_shape (SPTSpan *tspan, SPLayoutData *ly, NRPointF *cp, gboolean firstline, gboolean *inspace);
...
static void
sp_tspan_build (SPObject *object, SPDocument *doc, TheraNode *node)
{
	SPTSpan *tspan;
	SPString *string;
	TheraNode *thechild;
	...
	if (((SPObjectClass *) tspan_parent_class)->build)
		((SPObjectClass *) tspan_parent_class)->build (object, doc, node);
	...
	for (thechild = thera_node_get_first_child (node); thechild != NULL; thechild = thera_node_get_next_sibling (thechild)) {
		if (thera_node_get_type (thechild) == THERA_NODE_TEXT) break;
		if (thera_node_get_type (thechild) == THERA_NODE_CDATA) break;
	}

	/* fixme: We probably are not allowed to create node here */
	if (!thechild) {
		thechild = thera_document_new_node (doc->thedoc, THERA_NODE_TEXT, NULL);
		thera_node_add_child (node, thechild, NULL);
	}

	/* fixme: We should really pick up first child always */
	string = g_object_new (SP_TYPE_STRING, NULL);
	sp_object_ref (SP_OBJECT(string), object);
	g_object_unref (G_OBJECT (string));
	SP_OBJECT(string)->parent = object;
	/* fixme: We have to be sure that there are no children already present */
	SP_OBJECT(string)->next = object->children;
	object->children = SP_OBJECT(string);

	string->ly = &tspan->ly;
	// fixme: String does not work because it does not have element name
	sp_object_invoke_build (SP_OBJECT(string), doc, thechild, object, NULL, SP_OBJECT_IS_CLONED (object));
}
...
static SPObject *
sp_tspan_child_added (SPObject *object, TheraNode *rch, TheraNode *ref)
{
	SPTSpan *tspan;
	SPObject *child;
	...
	if (((SPObjectClass *) tspan_parent_class)->child_added)
		child = ((SPObjectClass *) tspan_parent_class)->child_added (object, rch, ref);

	g_return_val_if_fail (!child, child);

	if (!object->children && ((thera_node_get_type (rch) == THERA_NODE_TEXT) || (thera_node_get_type (rch) == THERA_NODE_CDATA))) {
		SPString *string;
		/* fixme: We should really pick up first child always */
		...
		g_object_unref (G_OBJECT (string));
		SP_OBJECT(string)->parent = object;
		SP_OBJECT(string)->next = NULL;

		string->ly = &tspan->ly;
		sp_object_invoke_build (object->children, object->document, rch, object, NULL, SP_OBJECT_IS_CLONED (object));
		child = SP_OBJECT(string);
	}
...
static unsigned int
sp_tspan_remove_child (SPObject *object, TheraNode *rch)
{
	SPTSpan *tspan;
	unsigned int ret;
...
static TheraNode *
sp_tspan_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags)
{
	SPTSpan *tspan;
	...
	if ((flags & SP_OBJECT_WRITE_BUILD) && !node) {
		node = thera_document_new_element (thedoc, "tspan");
	}

	if (tspan->ly.x.set) thera_node_set_double (node, "x", tspan->ly.x.computed);
	if (tspan->ly.y.set) thera_node_set_double (node, "y", tspan->ly.y.computed);
	if (tspan->ly.dx.set) thera_node_set_double (node, "dx", tspan->ly.dx.computed);
	if (tspan->ly.dy.set) thera_node_set_double (node, "dy", tspan->ly.dy.computed);
	if (tspan->ly.rotate_set) thera_node_set_double (node, "rotate", tspan->ly.rotate);
	if (flags & SP_OBJECT_WRITE_SODIPODI) {
		thera_node_set_attribute (node, "sodipodi:role", (tspan->role != SP_TSPAN_ROLE_UNSPECIFIED) ? "line" : NULL);
	}
	...
	if (flags & SP_OBJECT_WRITE_BUILD) {
		...
		/* TEXT element */
		rstr = thera_document_new_node (thedoc, THERA_NODE_TEXT, NULL);
		thera_node_set_text_content (rstr, text);
		g_free (text);

		thera_node_append_child (node, rstr);
	} else {
		unsigned char *utf8;
		utf8 = arikkei_ucs2_utf8_strdup (SP_STRING_UCHARS (object->children));
		thera_node_set_text_content (SP_OBJECT_REPR (object->children), utf8);
		free (utf8);
	}
	...
	if (((SPObjectClass *) tspan_parent_class)->write)
		((SPObjectClass *) tspan_parent_class)->write (object, thedoc, node, flags);

	return node;
}

static void
sp_tspan_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags)
{
	SPTSpan *tspan;
...
static void
sp_tspan_set_shape (SPTSpan *tspan, SPLayoutData *ly, NRPointF *cp, gboolean firstline, gboolean *inspace)
{
	SPStyle *style;
...
static void sp_text_build (SPObject * object, SPDocument * document, TheraNode * node);
...
static SPObject *sp_text_child_added (SPObject *object, TheraNode *rch, TheraNode *ref);
static unsigned int sp_text_remove_child (SPObject *object, TheraNode *rch);
...
static TheraNode *sp_text_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags);

static void sp_text_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags);
...
static void sp_text_write_transform (SPItem *item, TheraNode *node, NRMatrixF *transform);
...
static void
sp_text_build (SPObject *object, SPDocument *doc, TheraNode *node)
{
	SPText *text;
	SPObject *ref;
	TheraNode *rch;
	...
	if (((SPObjectClass *) text_parent_class)->build)
		((SPObjectClass *) text_parent_class)->build (object, doc, node);
	sp_object_read_attr (object, "x");
	sp_object_read_attr (object, "y");
	sp_object_read_attr (object, "dx");
	sp_object_read_attr (object, "dy");
	...
	for (rch = thera_node_get_first_child (node); rch != NULL; rch = thera_node_get_next_sibling (rch)) {
		if ((thera_node_get_type (rch) == THERA_NODE_TEXT) || (thera_node_get_type (rch) == THERA_NODE_CDATA)) {
			SPString *string;
			string = g_object_new (SP_TYPE_STRING, 0);
			if (ref) {
				ref->next = sp_object_attach_list_reref (object, SP_OBJECT (string), NULL);
			} else {
				object->children = sp_object_attach_list_reref (object, SP_OBJECT (string), NULL);
			}
			string->ly = &text->ly;
			sp_object_invoke_build (SP_OBJECT (string), doc, rch, object, ref, SP_OBJECT_IS_CLONED (object));
			ref = SP_OBJECT (string);
		} else if ((thera_node_get_type (rch) == THERA_NODE_ELEMENT) && !strcmp (thera_node_get_name (rch), "tspan")) {
			SPObject *child;
			child = g_object_new (SP_TYPE_TSPAN, 0);
			if (ref) {
				ref->next = sp_object_attach_list_reref (object, child, NULL);
			} else {
				object->children = sp_object_attach_list_reref (object, child, NULL);
			}
			sp_object_invoke_build (child, doc, rch, object, ref, SP_OBJECT_IS_CLONED (object));
			ref = child;
		} else {
			continue;
		}
...
sp_text_release (SPObject *object)
{
	...
	while (object->children) {
		object->children = sp_object_detach_list_unref (SP_OBJECT (object), NULL, object->children);
	}
...
static SPObject *
sp_text_child_added (SPObject *object, TheraNode *rch, TheraNode *ref)
{
	SPText *text;
	SPItem *item;
	SPObject *och, *prev;
	...
	if (((SPObjectClass *) text_parent_class)->child_added)
		och = ((SPObjectClass *) text_parent_class)->child_added (object, rch, ref);

	g_return_val_if_fail (!och, och);
	...
	if (ref != NULL) {
		...
		while (prev && (prev->node != ref)) prev = prev->next;
	}
	...
	if ((thera_node_get_type (rch) == THERA_NODE_TEXT) || (thera_node_get_type (rch) == THERA_NODE_CDATA)) {
		...
		string = g_object_new (SP_TYPE_STRING, 0);
		if (prev) {
			prev->next = sp_object_attach_list_reref (object, SP_OBJECT (string), prev->next);
		} else {
			object->children = sp_object_attach_list_reref (object, SP_OBJECT (string), object->children);
		}
		string->ly = &text->ly;
		sp_object_invoke_build (SP_OBJECT (string), object->document, rch, object, prev, SP_OBJECT_IS_CLONED (object));
		och = SP_OBJECT (string);
	} else if ((thera_node_get_type (rch) == THERA_NODE_ELEMENT) && !strcmp (thera_node_get_name (rch), "tspan")) {
		SPObject *child;
		child = g_object_new (SP_TYPE_TSPAN, 0);
		if (prev) {
			prev->next = sp_object_attach_list_reref (object, child, prev->next);
		} else {
			object->children = sp_object_attach_list_reref (object, child, object->children);
		}
		sp_object_invoke_build (child, object->document, rch, object, prev, SP_OBJECT_IS_CLONED (object));
		och = child;
	} else {
		och = NULL;
	}
...
static unsigned int
sp_text_remove_child (SPObject *object, TheraNode *rch)
{
	SPText *text;
	...
	if (((SPObjectClass *) text_parent_class)->remove_child) {
		if (!((SPObjectClass *) text_parent_class)->remove_child (object, rch)) return TRUE;
	}
...
static TheraNode *
sp_text_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags)
{
	SPText *text;
	SPObject *child;
	TheraNode *crepr;
	...
	if (flags & SP_OBJECT_WRITE_BUILD) {
		GSList *l;
		if (!node) node = thera_document_new_element (thedoc, "text");
		l = NULL;
		for (child = object->children; child != NULL; child = child->next) {
			if (SP_IS_TSPAN (child)) {
				...
				crepr = thera_document_new_node (thedoc, THERA_NODE_TEXT, NULL);
				thera_node_set_text_content (crepr, text);
				g_free (text);
			}
		}
		while (l) {
		...
		for (child = object->children; child != NULL; child = child->next) {
			if (SP_IS_TSPAN (child)) {
				sp_object_invoke_write (child, thedoc, SP_OBJECT_REPR (child), flags);
			} else {
	...
	if (text->ly.x.set) thera_node_set_double (node, "x", text->ly.x.computed);
	if (text->ly.y.set) thera_node_set_double (node, "y", text->ly.y.computed);
	if (text->ly.dx.set) thera_node_set_double (node, "dx", text->ly.dx.computed);
	if (text->ly.dy.set) thera_node_set_double (node, "dy", text->ly.dy.computed);
	if (text->ly.rotate_set) thera_node_set_double (node, "rotate", text->ly.rotate);

	if (((SPObjectClass *) (text_parent_class))->write)
		((SPObjectClass *) (text_parent_class))->write (object, thedoc, node, flags);

	return node;
}

static void
sp_text_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags)
{
	SPText *text;
	SPItem *child;
	SPObject *o;
	...
	for (o = ((SPObject *) text)->children; o != NULL; o = o->next) {
		NRMatrixF a;
		child = SP_ITEM (o);
		nr_matrix_multiply_fff (&a, &child->transform, transform);
	...
	while (child != NULL) {
		...
		if (SP_IS_TSPAN (child)) {
			...
			case SP_TSPAN_ROLE_LINE:
				...
				thera_node_set_double (SP_OBJECT_REPR (tspan), "x", cp.x);
				thera_node_set_double (SP_OBJECT_REPR (tspan), "y", cp.y);
...
static void
sp_text_write_transform (SPItem *item, TheraNode *node, NRMatrixF *t)
{
	SPText *text;
	NRMatrixF i2p, p2i;
	gdouble px, py, x, y;
	...
	thera_node_set_double (node, "x", x);
	thera_node_set_double (node, "y", y);
	for (child = ((SPObject *) text)->children; child != NULL; child = child->next) {
		if (SP_IS_TSPAN (child)) {
			SPTSpan *tspan;
			tspan = SP_TSPAN (child);
			if (tspan->ly.x.set || tspan->ly.y.set) {
				...
				x = NR_MATRIX_DF_TRANSFORM_X (&p2i, px, py);
				y = NR_MATRIX_DF_TRANSFORM_Y (&p2i, px, py);
				/* fixme: This is wrong - what if node != SP_OBJECT_REPR (text) */
				thera_node_set_double (SP_OBJECT_REPR (tspan), "x", x);
				thera_node_set_double (SP_OBJECT_REPR (tspan), "y", y);
			}
		}
	}
	*t = i2p;
...
sp_text_set_repr_text_multiline (SPText *text, const guchar *str)
{
	...
	doc = SP_OBJECT_DOCUMENT (text);
	node = SP_OBJECT_REPR (text);
	style = SP_OBJECT_STYLE (text);

	if (!str) str = "";
	content = g_strdup (str);

	thera_node_set_text_content (SP_OBJECT_REPR (text), NULL);
	while (thera_node_get_first_child (node)) {
		thera_node_remove_child (node, thera_node_get_first_child (node));
	}
	...
	while (p) {
		TheraNode *rtspan, *rstr;
		guchar *e;
		e = strchr (p, '\n');
		if (e) *e = '\0';
		rtspan = thera_document_new_element (doc->thedoc, "tspan");
		thera_node_set_double (rtspan, "x", cp.x);
		thera_node_set_double (rtspan, "y", cp.y);
		if (style->writing_mode.computed == SP_CSS_WRITING_MODE_TB) {
		...
		thera_node_set_attribute (rtspan, "sodipodi:role", "line");
		rstr = thera_document_new_node (doc->thedoc, THERA_NODE_TEXT, NULL);
		thera_node_set_text_content (rstr, p);
		thera_node_add_child (rtspan, rstr, NULL);
		thera_node_append_child (node, rtspan);
		p = (e) ? e + 1 : NULL;
	}
...
sp_text_append_line (SPText *text)
{
	...
	if (style->writing_mode.computed == SP_CSS_WRITING_MODE_TB) {
		/* fixme: real line height */
		/* fixme: What to do with mixed direction tspans? */
		thera_node_set_double (rtspan, "x", cp.x - style->font_size.computed);
		thera_node_set_double (rtspan, "y", cp.y);
	} else {
		thera_node_set_double (rtspan, "x", cp.x);
		thera_node_set_double (rtspan, "y", cp.y + style->font_size.computed);
	}
	thera_node_set_attribute (rtspan, "sodipodi:role", "line");

	/* Create TEXT */
	rstring = thera_document_new_node (doc->thedoc, THERA_NODE_TEXT, NULL);
	thera_node_set_text_content (rstring, "");
	thera_node_add_child (rtspan, rstring, NULL);
	/* Append to text */
	thera_node_append_child (SP_OBJECT_REPR (text), rtspan);

	return (SPTSpan *) sp_document_lookup_id (SP_OBJECT_DOCUMENT (text), thera_node_get_attribute (rtspan, "id"));
}
...
sp_text_insert_line (SPText *text, gint pos)
{
	...
	thera_node_add_child (SP_OBJECT_REPR (text), rtspan, SP_OBJECT_REPR (child));

	if (string->uchars) {
		NRUShort *ip;
		unsigned char *utf8;
		ip = string->uchars + (pos - string->start);
		utf8 = arikkei_ucs2_utf8_strdup (ip);
		thera_node_set_text_content (rstring, utf8);
		free (utf8);
		*ip = '\0';
		utf8 = arikkei_ucs2_utf8_strdup (string->uchars);
		thera_node_set_text_content (SP_OBJECT_REPR (string), utf8);
		free (utf8);
	}
...
sp_text_append (SPText *text, const guchar *utf8)
{
	...
	if (!((SPObject *) text)->children) {
		...
		thera_node_add_child (rtspan, rstring, NULL);
		/* Add string */
		thera_node_add_child (SP_OBJECT_REPR (text), rtspan, NULL);
	...
	content = thera_node_get_text_content (SP_OBJECT_REPR (string));
	...
	*(p + clen + ulen) = '\0';
	...
	thera_node_set_text_content (SP_OBJECT_REPR (string), p);
...
sp_text_insert (SPText *text, gint pos, const guchar *utf8, gboolean preservews)
{
	...
	int ucs2len, slen, ulen, newpos, i;
	...
	ucs2len = (string->uchars) ? arikkei_ucs2_strlen (string->uchars) : 0;
	slen = ip - string->uchars;
	ulen = g_utf8_strlen (utf8, -1);
	ucs2 = g_utf8_to_utf16 (utf8, -1, NULL, NULL, NULL);
	new = g_new (NRUShort, ucs2len + ulen + 1);
...
sp_text_delete (SPText *text, gint start, gint end)
{
	...
	if (schild != echild) {
		...
		cl = g_slist_prepend (cl, SP_OBJECT_REPR (child));
		while (cl) {
			thera_node_unparent ((TheraNode *) cl->data);
...
void
sp_text_get_cursor_coords (SPText *text, gint position, NRPointF *p0, NRPointF *p1)
{
	SPObject *child;
	SPString *string;
	gfloat x, y;
...
sp_text_get_child_by_position (SPText *text, gint pos)
{
	...
	for (child = ((SPObject *) text)->children; child && child->next; child = child->next) {
		...
		if (pos <= (int) (string->start + string->ulen)) return child;
	}
