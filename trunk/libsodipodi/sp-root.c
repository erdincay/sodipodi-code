...
static void sp_root_build (SPObject *object, SPDocument *document, TheraNode *node);
...
static SPObject *sp_root_child_added (SPObject *object, TheraNode *child, TheraNode *ref);
static unsigned int sp_root_remove_child (SPObject *object, TheraNode *child);
static void sp_root_update (SPObject *object, SPCtx *ctx, unsigned int flags);
static void sp_root_modified (SPObject *object, unsigned int flags);
static TheraNode *sp_root_write (SPObject *object, TheraDocument *thedoc, TheraNode *repr, unsigned int flags);

static NRArenaItem *sp_root_show (SPItem *item, NRArena *arena, unsigned int key, unsigned int flags);
static void sp_root_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags);
...
static void
sp_root_build (SPObject *object, SPDocument *document, TheraNode *node)
{
	SPGroup *group;
	SPRoot *root;
	SPObject *o;
	...
	if (thera_node_get_attribute (node, "sodipodi:docname") || thera_node_get_attribute (node, "SP-DOCNAME")) {
...
sp_root_set (SPObject *object, unsigned int key, const unsigned char *value)
{
	...
	case SP_ATTR_VERSION:
		if (value) {
			root->version = (float) sp_svg_atof (value);
		} else {
			root->version = 1.0;
		}
		break;
...
#if 1
	case SP_ATTR_X:
		if (sp_svg_length_read_uff (value, &unit, &root->x.value, &root->x.computed) &&
		...
		break;
	case SP_ATTR_Y:
		if (sp_svg_length_read_uff (value, &unit, &root->y.value, &root->y.computed) &&
		    /* fixme: These are probably valid, but require special treatment (Lauris) */
		    (unit != SP_SVG_UNIT_EM) &&
		    (unit != SP_SVG_UNIT_EX)) {
			root->y.set = TRUE;
		...
		break;
	case SP_ATTR_WIDTH:
		if (sp_svg_length_read_uff (value, &unit, &root->width.value, &root->width.computed) &&
		    /* fixme: These are probably valid, but require special treatment (Lauris) */
		    (unit != SP_SVG_UNIT_EM) &&
		    (unit != SP_SVG_UNIT_EX) &&
		...
		break;
	case SP_ATTR_HEIGHT:
		if (sp_svg_length_read_uff (value, &unit, &root->height.value, &root->height.computed) &&
		    /* fixme: These are probably valid, but require special treatment (Lauris) */
		    (unit != SP_SVG_UNIT_EM) &&
		    (unit != SP_SVG_UNIT_EX) &&
...
static SPObject *
sp_root_child_added (SPObject *object, TheraNode *child, TheraNode *ref)
{
	SPRoot *root;
	SPGroup *group;
	SPObject *co;
	...
	if (((SPObjectClass *) (parent_class))->child_added)
		co = ((SPObjectClass *) (parent_class))->child_added (object, child, ref);
...
static unsigned int
sp_root_remove_child (SPObject * object, TheraNode *child)
{
	SPRoot * root;
	SPObject * co;
	const gchar * id;
...
sp_root_update (SPObject *object, SPCtx *ctx, guint flags)
{
	...
	if (root->x.unit == SP_SVG_UNIT_PERCENT) {
		root->x.computed = root->x.value * (float) (ictx->vp.x1 - ictx->vp.x0);
	}
	if (root->y.unit == SP_SVG_UNIT_PERCENT) {
		root->y.computed = root->y.value * (float) (ictx->vp.y1 - ictx->vp.y0);
	}
	if (root->width.unit == SP_SVG_UNIT_PERCENT) {
		root->width.computed = root->width.value * (float) (ictx->vp.x1 - ictx->vp.x0);
	}
	if (root->height.unit == SP_SVG_UNIT_PERCENT) {
		root->height.computed = root->height.value * (float) (ictx->vp.y1 - ictx->vp.y0);
	}
...
static TheraNode *
sp_root_write (SPObject *object, TheraDocument *thedoc, TheraNode *repr, guint flags)
{
	SPRoot *root;
	SPVPGroup *vpgroup;
	...
	if ((flags & SP_OBJECT_WRITE_BUILD) && !repr) {
		repr = thera_document_new_node (thedoc, THERA_NODE_ELEMENT, "svg");
	}

	thera_node_set_attribute (repr, "xmlns", "http://www.w3.org/2000/svg");
	thera_node_set_attribute (repr, "xmlns:xlink", "http://www.w3.org/1999/xlink");

	if (flags & SP_OBJECT_WRITE_SODIPODI) {
		thera_node_set_attribute (repr, "xmlns:sodipodi", "http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd");
		thera_node_set_double (repr, "sodipodi:version", (double) root->sodipodi / 100.0);
	}

	thera_node_set_attribute (repr, "version", "1.0");
	thera_node_set_double (repr, "x", vpgroup->x.computed);
	thera_node_set_double (repr, "y", vpgroup->y.computed);
	thera_node_set_double (repr, "width", vpgroup->width.computed);
	thera_node_set_double (repr, "height", vpgroup->height.computed);
	thera_node_set_attribute (repr, "viewBox", thera_node_get_attribute (object->node, "viewBox"));

	if (((SPObjectClass *) (parent_class))->write)
		((SPObjectClass *) (parent_class))->write (object, thedoc, repr, flags);
...
static void
sp_root_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags)
{
	SPRoot *root;
	SPVPGroup *vpgroup;
	NRMatrixF a;
	...
	nr_matrix_multiply_fdf (&a, &vpgroup->c2p, transform);
