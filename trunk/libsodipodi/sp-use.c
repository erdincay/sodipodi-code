...
static void sp_use_build (SPObject * object, SPDocument * document, TheraNode * node);
...
static TheraNode *sp_use_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags);
static void sp_use_update (SPObject *object, SPCtx *ctx, guint flags);
static void sp_use_modified (SPObject *object, guint flags);

static void sp_use_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags);
...
static void
sp_use_build (SPObject * object, SPDocument * document, TheraNode * node)
{
	SPUse * use;
	...
	if (use->href) {
		...
		if (refobj) {
			...
			type = sp_node_type_lookup (childrepr);
			g_return_if_fail (type > G_TYPE_NONE);
			if (g_type_is_a (type, SP_TYPE_ITEM)) {
				SPObject *childobj;
				childobj = g_object_new (type, 0);
				use->child = sp_object_attach_list_reref (object, childobj, NULL);
				sp_object_invoke_build (childobj, document, childrepr, object, NULL, TRUE);
			}
		}
	}
}
...
sp_use_release (SPObject *object)
{
	...
	if (use->child) {
		use->child = sp_object_detach_list_unref (SP_OBJECT (object), NULL, use->child);
	}
...
static TheraNode *
sp_use_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags)
{
	SPUse *use;
	...
	if ((flags & SP_OBJECT_WRITE_BUILD) && !node) {
		node = thera_document_new_element (thedoc, "use");
	}

	thera_node_set_attribute (node, "id", object->id);
	thera_node_set_attribute (node, "xlink:href", use->href);
	thera_node_set_double (node, "x", use->x.computed);
	thera_node_set_double (node, "y", use->y.computed);
	thera_node_set_double (node, "width", use->width.computed);
	thera_node_set_double (node, "height", use->height.computed);

	return node;
}

static void
sp_use_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags)
{
	SPUse * use;
	...
	if (use->child && SP_IS_ITEM (use->child)) {
		...
		nr_matrix_multiply_fff (&ct, &t, transform);
		nr_matrix_multiply_fff (&ct, &child->transform, &ct);
...
sp_use_href_changed (SPUse * use)
{
	...
	if (use->child) {
		use->child = sp_object_detach_list_unref (SP_OBJECT (use), NULL, use->child);
	}
	...
	if (use->href) {
		...
		if (refobj) {
			...
			if (g_type_is_a (type, SP_TYPE_ITEM)) {
				...
				SPItemView * v;
				childobj = g_object_new (type, 0);
				use->child = sp_object_attach_list_reref (SP_OBJECT (use), childobj, NULL);
				sp_object_invoke_build (childobj, SP_OBJECT (use)->document, node, SP_OBJECT(use), NULL, TRUE);
