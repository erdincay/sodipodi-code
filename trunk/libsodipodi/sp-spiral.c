...
static void sp_spiral_build (SPObject * object, SPDocument * document, TheraNode * node);
static TheraNode *sp_spiral_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags);
...
static void
sp_spiral_build (SPObject * object, SPDocument * document, TheraNode * node)
{
...
static TheraNode *
sp_spiral_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags)
{
	SPSpiral *spiral;
	char *d;
	...
	if ((flags & SP_OBJECT_WRITE_BUILD) && !node) {
		node = thera_document_new_element (thedoc, "path");
	}
	...
	if (flags & SP_OBJECT_WRITE_SODIPODI) {
		 ...
		 */
		thera_node_set_attribute (node, "sodipodi:type", "spiral");
		thera_node_set_double (node, "sodipodi:cx", spiral->cx);
		thera_node_set_double (node, "sodipodi:cy", spiral->cy);
		thera_node_set_double (node, "sodipodi:expansion", spiral->exp);
		thera_node_set_double (node, "sodipodi:revolution", spiral->revo);
		thera_node_set_double (node, "sodipodi:radius", spiral->rad);
		thera_node_set_double (node, "sodipodi:argument", spiral->arg);
		thera_node_set_double (node, "sodipodi:t0", spiral->t0);
	}

	d = sp_svg_write_path (((SPShape *) spiral)->curve);
	thera_node_set_attribute (node, "d", d);
	g_free (d);

	if (((SPObjectClass *) (parent_class))->write)
		((SPObjectClass *) (parent_class))->write (object, thedoc, node, flags | SP_SHAPE_WRITE_PATH);
...
sp_spiral_set (SPObject *object, unsigned int key, const unsigned char *value)
{
	...
	case SP_ATTR_SODIPODI_CX:
		if (!sp_svg_length_read_uff (value, &unit, NULL, &spiral->cx) ||
		    (unit == SP_SVG_UNIT_EM) ||
		    (unit == SP_SVG_UNIT_EX) ||
	...
	case SP_ATTR_SODIPODI_CY:
		if (!sp_svg_length_read_uff (value, &unit, NULL, &spiral->cy) ||
		    (unit == SP_SVG_UNIT_EM) ||
		    (unit == SP_SVG_UNIT_EX) ||
		    (unit == SP_SVG_UNIT_PERCENT)) {
			spiral->cy = 0.0;
	...
	case SP_ATTR_SODIPODI_RADIUS:
		if (!sp_svg_length_read_uff (value, &unit, NULL, &spiral->rad) ||
		    (unit != SP_SVG_UNIT_EM) ||
		    (unit != SP_SVG_UNIT_EX) ||
		    (unit != SP_SVG_UNIT_PERCENT)) {
...
sp_spiral_get_xy (SPSpiral *spiral, gdouble t, NRPointF *p)
{
	...
	arg = NR_2PI_F * spiral->revo * t + spiral->arg;
