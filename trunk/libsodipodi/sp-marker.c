...
static void sp_marker_build (SPObject *object, SPDocument *document, TheraNode *node);
...
static TheraNode *sp_marker_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags);
...
static void sp_marker_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags);
static unsigned int sp_marker_get_extra_transform (SPItem *item, NRMatrixD *transform);
...
sp_marker_class_init (SPMarkerClass *klass)
{
	...
	sp_item_class->get_extra_transform = sp_marker_get_extra_transform;
...
static void
sp_marker_build (SPObject *object, SPDocument *document, TheraNode *node)
{
	SPGroup *group;
	SPMarker *marker;
...
static TheraNode *
sp_marker_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags)
{
	SPMarker *marker;
	...
	if ((flags & SP_OBJECT_WRITE_BUILD) && !node) {
		node = thera_document_new_node (thedoc, THERA_NODE_ELEMENT, "marker");
	}

	if (marker->markerUnits_set) {
		if (marker->markerUnits == SP_MARKER_UNITS_STROKEWIDTH) {
			thera_node_set_attribute (node, "markerUnits", "strokeWidth");
		} else {
			thera_node_set_attribute (node, "markerUnits", "userSpaceOnUse");
		}
	} else {
		thera_node_set_attribute (node, "markerUnits", NULL);
	}
	if (marker->refX.set) {
		thera_node_set_double (node, "refX", marker->refX.computed);
	} else {
		thera_node_set_attribute (node, "refX", NULL);
	}
	if (marker->refY.set) {
		thera_node_set_double (node, "refY", marker->refY.computed);
	} else {
		thera_node_set_attribute (node, "refY", NULL);
	}
	if (marker->markerWidth.set) {
		thera_node_set_double (node, "markerWidth", marker->markerWidth.computed);
	} else {
		thera_node_set_attribute (node, "markerWidth", NULL);
	}
	if (marker->markerHeight.set) {
		thera_node_set_double (node, "markerHeight", marker->markerHeight.computed);
	} else {
		thera_node_set_attribute (node, "markerHeight", NULL);
	}
	if (marker->orient_set) {
		if (marker->orient_auto) {
			thera_node_set_attribute (node, "orient", "auto");
		} else {
			thera_node_set_double (node, "orient", marker->orient);
		}
	} else {
		thera_node_set_attribute (node, "orient", NULL);
	}
	/* fixme: */
	thera_node_set_attribute (node, "viewBox", thera_node_get_attribute (object->node, "viewBox"));
	thera_node_set_attribute (node, "preserveAspectRatio", thera_node_get_attribute (object->node, "preserveAspectRatio"));
...
static void
sp_marker_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags)
{
	/* Break propagation */
}

static unsigned int
sp_marker_get_extra_transform (SPItem *item, NRMatrixD *transform)
{
	SPMarker *marker;
		...
		if (v->key == key) {
			...
			if (v->items[pos]) {
				...
				if (marker->orient_auto) {
					...
					nr_matrix_f_set_rotate (&m, (float) (marker->orient * M_PI / 180.0));
					m.c[4] = base->c[4];
					m.c[5] = base->c[5];
				}
				if (marker->markerUnits == SP_MARKER_UNITS_STROKEWIDTH) {
                      ...
                      NRRectF *bbox)
{
	...
	for (v = marker->views; v != NULL; v = v->next) {
		...
		if (v->key == key) {
			...
			x = (float) fabs (NR_MATRIX_DF_TRANSFORM_X(&m,1,1));
			y = (float) fabs (NR_MATRIX_DF_TRANSFORM_Y(&m,1,1));
