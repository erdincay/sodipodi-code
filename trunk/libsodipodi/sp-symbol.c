...
static void sp_symbol_build (SPObject *object, SPDocument *document, TheraNode *node);
...
static SPObject *sp_symbol_child_added (SPObject *object, TheraNode *child, TheraNode *ref);
static unsigned int sp_symbol_remove_child (SPObject *object, TheraNode *child);
...
static TheraNode *sp_symbol_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags);
...
static void sp_symbol_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags);
...
static void
sp_symbol_build (SPObject *object, SPDocument *document, TheraNode *node)
{
	SPGroup *group;
	SPSymbol *symbol;
...
static SPObject *
sp_symbol_child_added (SPObject *object, TheraNode *thechild, TheraNode *theref)
{
	SPObject *child;
...
static unsigned int
sp_symbol_remove_child (SPObject * object, TheraNode * child)
{
	SPSymbol *symbol;
...
static TheraNode *
sp_symbol_write (SPObject *object, TheraDocument *thedoc, TheraNode *node, guint flags)
{
	SPSymbol *symbol;
	...
	if ((flags & SP_OBJECT_WRITE_BUILD) && !node) {
		node = thera_document_new_element (thedoc, "symbol");
	}
...
static void
sp_symbol_bbox (SPItem *item, NRRectF *bbox, const NRMatrixF *transform, unsigned int flags)
{
	SPSymbol *symbol;
	NRMatrixF a;
	...
	if (SP_OBJECT_IS_CLONED (symbol)) {
		...
		nr_matrix_multiply_fdf (&a, &symbol->c2p, transform);
