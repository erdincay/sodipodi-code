...
static const SPStyleProp props[] = {
	...
	{SP_ATTR_SODIPODI_INVISIBLE, "sodipodi:invisible"},
...
static ArikkeiDict keydict;
static ArikkeiDict iddict;
...
initialize (void)
{
	if (!initialized) {
		unsigned int i;
		arikkei_dict_setup_string (&keydict, 71);
		arikkei_dict_setup_int (&iddict, 71);
		for (i = 1; i < n_attrs; i++) {
			assert (props[i].code == i);
			arikkei_dict_insert (&keydict, props[i].name, ((char *) NULL) + props[i].code);
			arikkei_dict_insert (&iddict, ((char *) NULL) + props[i].code, props[i].name);
		}
		initialized = 1;
	}
}
...
sp_attribute_lookup (const unsigned char *key)
{
	...
	return (char *) arikkei_dict_lookup (&keydict, key) - (char *) NULL;
}
...
sp_attribute_name (unsigned int id)
{
	...
	return arikkei_dict_lookup (&iddict, ((char *) NULL) + id);
}

unsigned int
sp_attribute_register (unsigned int id, const unsigned char *key)
{
	initialize ();

	if (arikkei_dict_exists (&keydict, key)) return 0;
	if (arikkei_dict_exists (&iddict, ((char *) NULL) + id)) return 0;

	arikkei_dict_insert (&keydict, key, ((char *) NULL) + id);
	arikkei_dict_insert (&iddict, ((char *) NULL) + id, key);
